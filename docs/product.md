# Product

One account. Several focused apps. One inexpensive suite subscription, with AI as BYOK and/or a small included quota — not unlimited tokens inside £4.99.

**Job:** help someone practise skills, eat, train, and (later) look after mood, without four other subscriptions and without pretending the app is a clinician, dietitian, or PT.

LifeQuest is the hub. Other apps keep their own data. Cross-app XP is later and opt-in. Public pride is **proof** (cleared gates, finished sessions), not a raw level. Rest is not failure. Social, when it exists, is opt-in milestone cards — not a default leaderboard.

AI is a pace-aware coach grounded in *this user’s* logs. Keys stay server-side, encrypted. Never scrape third-party recipe sites for a “stock library.”

Horizon (not a backlog to implement from this sentence): fasting, plate-photo *confirm*, restaurant lookup, five focus vibes, GPS/watches on native/PWA, sleep, household pantry. See git history if you need the long version; do not grow this file into a tracker.
