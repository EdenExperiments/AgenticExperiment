...
Last updated: 2026-05-15 (Cursor Lab eval flow plan revised — F-061)
...
| F-061 | Cursor Lab: LLM-as-judge eval flow (plan) | Ops/CI | ready-for-build | Plan at `docs/guides/cursor-lab-eval-flow-plan.md`. Same monorepo `apps/cursor-lab/`: Python + Node bridge to `@cursor/sdk` (`Agent.prompt` default, optional stream mode for `tool_call` transcripts per public SDK docs), versioned **JSON judge verdicts** (rationales + evidence + suggestions), `lab/registry.yaml` lifecycle (`draft`→`approved`) + **`materialize-prod`**, sandbox profiles (`strict`/`bundled`/`workspace_slice`), fingerprint cache, pnpm `package.json` + ripgrep bootstrap notes. |
